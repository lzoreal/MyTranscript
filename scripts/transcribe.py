...
def safe_filename(title):
    ...
    return (
        "".join(c if c in keep else "_" for c in title)
        .strip()
        .replace(" ", "_")[:80]
    )
...
def generate_podcast_feed(pc_state):
    ...
    feed_url = (
        f"{BASE_URL}/{PODCAST_SLUG}/feed.xml"
    )
    ...
    for parent in (channel, root):
        for atom_link in parent.findall(
            f"{{{atom_uri}}}link"
        ):
    ...
    for item in channel.findall(
        ...
        actual_audio_url = episode_state.get(
            "audio_url"
        )
        ...
        vtt_filename = episode_state.get(
            "vtt_filename"
        )
...
def main():
    if (
        ...
        print(
            "❌ 错误：需要设置 "
            "PODCAST_SLUG, FEED_URL, BASE_URL"
        )
